...
if __name__ == "__main__":
    ...
    print(f"Running training for plots... (Seed: {args.seed}, Episodes: {num_episodes})")
    for _ in tqdm(range(num_episodes)):
        # Run random agent
        r_metrics = run_episode(None, agent_type="random")
        random_rewards.append(r_metrics["total_reward"])
        ...
        if args.quick_run or _ == num_episodes - 1:
            tqdm.write("\n=== EPISODE SUMMARY ===")
            tqdm.write(f"Total Reward: {t_metrics['total_reward']:.2f}")
            tqdm.write(f"Trust Calibration Avg: {t_metrics['trust_calibration']:.2f}")
            tqdm.write(f"Alliances Formed: {t_metrics['alliances_formed']}")
            tqdm.write("=======================\n")

    # Pre-processing safeguards
    trained_rewards = np.clip(trained_rewards, -5, 5)
    random_rewards = np.clip(random_rewards, -5, 5)
    heuristic_rewards = np.clip(heuristic_rewards, -5, 5)
    
    trust_scores = [max(0, min(1, t)) for t in trust_scores]
    heuristic_trust = [max(0, min(1, t)) for t in heuristic_trust]
    
    # Calculate Metrics
    avg_trained_reward = np.mean(trained_rewards)
    avg_heuristic_reward = np.mean(heuristic_rewards)
    avg_trained_trust = np.mean(trust_scores)
    avg_heuristic_trust = np.mean(heuristic_trust)
    
    print("\n" + "="*50)
    print("📊 PERFORMANCE METRICS")
    print("="*50)
    print(f"Average reward improved from {avg_heuristic_reward:.2f} → {avg_trained_reward:.2f}")
    print(f"Trust calibration improved from {avg_heuristic_trust:.2f} → {avg_trained_trust:.2f}")
    print(f"Average Alliances Formed per Episode: Trained ({alliances_formed['trained']/num_episodes:.1f}), Heuristic ({alliances_formed['heuristic']/num_episodes:.1f}), Random ({alliances_formed['random']/num_episodes:.1f})")
    print("="*50 + "\n")
    
    # Plot 1: Reward Curve
    smooth_trained = smooth(trained_rewards, 10)
    smooth_random = smooth(random_rewards, 10)
    smooth_heuristic = smooth(heuristic_rewards, 10)

    plt.figure(figsize=(8,5))

    plt.plot(trained_rewards, alpha=0.1)
    plt.plot(random_rewards, alpha=0.1)
    plt.plot(heuristic_rewards, alpha=0.1)

    plt.plot(smooth_trained, linewidth=2, label="Trained Agent")
    plt.plot(smooth_heuristic, linewidth=2, label="Heuristic Agent")
    plt.plot(smooth_random, linestyle="--", linewidth=2, label="Random Agent")

    plt.xlabel("Episodes")
    plt.ylabel("Total Reward")
    plt.title("Training Progress: Learned Diplomacy vs Baselines")
    plt.legend()
    plt.grid(alpha=0.3)
    plt.tight_layout()
    plt.savefig("reward_curve.png", dpi=300)
    ...
    smooth_h_trust = smooth(heuristic_trust, 10)

    plt.figure(figsize=(8,5))

    plt.plot(trust_scores, alpha=0.1)
    plt.plot(heuristic_trust, alpha=0.1)

    plt.plot(smooth_trust, linewidth=2, label="Trained Trust Calib.")
    plt.plot(smooth_h_trust, linewidth=2, label="Heuristic Trust Calib.", linestyle="--")

    plt.xlabel("Episodes")
    plt.ylabel("Trust Calibration Score")
    plt.title("Learning Theory-of-Mind: Intent Prediction")
    plt.legend()
    plt.grid(alpha=0.3)
    plt.ylim(0, 1.0)
    ...
    print(f"Reward Improvement:  {Colors.OKGREEN}{avg_heuristic_reward:.2f} → {avg_trained_reward:.2f}{Colors.ENDC}")
    print(f"Trust Calibration:   {Colors.WARNING}{avg_heuristic_trust:.2f} → {avg_trained_trust:.2f}{Colors.ENDC}")
    print(f"Alliances Formed:    {Colors.OKGREEN}{alliances_formed['heuristic']/num_episodes:.1f} → {alliances_formed['trained']/num_episodes:.1f}{Colors.ENDC}")
    print(f"{Colors.HEADER}{Colors.BOLD}=============================={Colors.ENDC}\n")
